    ...
    public function showMyCharacter(){
    	...
    	if($user != null){
    		...
    		if($character != null){
				$skill_handout_objects = array();
				$char_docs = array();
				$char_docs[] = ['char_doc_name' => 'test1.pdf'];

				if(Storage::disk('chardocs')->exists('/'.$character->id)){
					$char_docs = glob(storage_path('app/chardocs/'.$character->id).'//*.pdf');
					$char_docs[] = ['char_doc_name' => 'test2.pdf'];
				}
	...
	public function doUploadCharDoc(){
		...
		if(Input::hasFile('char_doc_selection')) {
			...
			if($validator->fails()){
				...
				return view('/character/charDocAdded', ['charId' => $charId]);
			}
		} else {
			// No file selected
